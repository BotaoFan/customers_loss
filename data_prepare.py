...
if __name__=='__main__':
    #Set data dir path
    dir_path='/Users/fan/PycharmProjects/machine_learning/customer_loss/'
    data_dir_name='raw_data_customers_loss_17-5_17-8'
    ...
    #======Model Training======
    paras = {'learning_rate':0.1,'n_estimators':100,'min_child_weight':3,'max_depth':4,
              'seed': 0,'gamma':0.6,'subsample':0.8,'colsample_bytree':1,'random_state':42}
...
#-----------
    test_result_cluster_list=[]
    model_paras_list=[]
    for i in range(8):
        print '=====Training Cluster %d=====' %i
        train_temp=train_all_features.loc[train_all_features['cluster_id']==i,]
        test_temp=test_all_features.loc[test_all_features['cluster_id']==i,]
        train_x = train_temp.drop(columns=['y','cluster_id']).values
        train_y = pd.DataFrame(train_temp['y']).values
        test_x = test_temp.drop(columns=['y','cluster_id']).values
        test_y = pd.DataFrame(test_temp['y']).values
        xgbTrain=XGBoostTrainer(train_x,train_y)
        paras_temp=xgbTrain.model_train(init_paras,test_paras)
        model_temp=xgb.XGBClassifier(**paras_temp)
        model_temp.fit(train_x,train_y)

        test_pred_y = model_temp.predict_proba(test_x)
        test_pred_y = test_pred_y[:, 1]
        roc_auc_score(test_y, test_pred_y)
        test_pred_y_df = pd.DataFrame(test_pred_y, columns=['prob'])
        test_y_df = pd.DataFrame(test_y, columns=['true'])
        test_result_df = pd.concat([test_pred_y_df, test_y_df], axis=1)
        test_result_df.sort_values('prob', inplace=True, ascending=False)
        test_result_cluster_list.append(test_result_df.copy())
        model_paras_list.append(paras_temp.copy())
